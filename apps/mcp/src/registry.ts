...
export const TOOL_DEFINITIONS = [
  {
    ...
    description:
      "**Use when the question is conceptual rather than literal** — e.g. 'where do we debounce websocket reconnects', 'how is tenant id propagated', 'where does this feature get documented'. **Beats `grep`** when no single token captures the question: the vector branch surfaces relevant chunks that contain *no token-level match* (a doc that *describes* a feature without naming the literal token, a function whose body implements the idea without using your query word). Each hit also includes the **surrounding chunk** (function body, class body, doc section), so in most cases the snippet alone is enough to act without a follow-up `Read` of the file — saves context-window space. Pass `coverage: true` for refactor planning ('what else touches X'): after the ranked list, top hits are expanded with their callers/importers via the symbol cross-ref graph (each expansion carries a `coverageReason`). **Not** the right tool for exhaustive literal-string audits — the lexical branch tokenizes path-like strings into segments and returns ranked partial-match chunks, not a grep-style file list. For that, use `find_literal`. For exact-symbol cross-references, use `find_usages`. Each result carries `absPath`, `relPath`, `projectName`, line range, score, snippet, `matchReasons`, `analyzer` (AST metadata), and `sources` (vector|lexical). Pass `path` to scope to a project or subtree. Highest-leverage on unfamiliar or large codebases; on a small repo you've already explored, `grep` + `Read` may be faster end-to-end. The response's `indexHealth.reconciling` field signals partial results when a re-index is in flight.",
    inputSchema: {
      type: "object",
      required: ["query"],
      properties: {
  ...
  },
  {
    name: "find_literal",
    description:
      "**Use when you need every occurrence of a literal substring across the indexed workspace** — 'every file referencing agents/foo.md', 'every config still pointing at the old URL', 'where is the deprecated constant still used'. Returns one row per matched line with `relPath`, `line`, `column`, `lineText`, plus the surrounding chunk metadata. **Does NOT beat `grep` for safety-critical audits.** The scan operates on indexed chunk text, so chunker gaps (#360) and excluded-by-default directories (`.git`, `node_modules`, build outputs) are blind spots. Use when an indexed-chunk view is sufficient and you want structured per-line responses with surrounding chunk context. The response always includes a `coverageNote` — read it. For audit-critical questions ('is this stale URL referenced anywhere'), supplement with `rg <pattern>` to verify. Complements `search_workspace` (ranked / semantic) and `find_usages` (exact symbol cross-ref). **0-hit semantics:** check `indexHealth.reconciling`. If `true`, retry after the re-index. If `false`, the substring either isn't in indexed chunks or this repo isn't indexed (verify with `workspace_status`). Highest-leverage on unfamiliar or large codebases; for small repos, `rg` may be faster end-to-end.",
    inputSchema: {
      type: "object",
      required: ["pattern"],
      properties: {
